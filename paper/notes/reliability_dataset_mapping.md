...
已确认(2026-07-18,用户口头):NdFeB **2000 目**(≈6.5 µm);PDMS
base:curing **10:1**;预取向与磁化"应该就是一项"(理解为:无固化中预取向,
仅一次充磁——**待最终确认措辞**)。

追加确认(2026-07-18):器件 NdFeB 组分 = **60 wt%**(用户回忆为 60 或
70,先按 60 写,稿内留 confirm 标记,需对照制备记录最终定)。Ecoflex 牌号
= 00-30(拉伸数据确认)。

仍待确认:
1. NdFeB 牌号/供应商;60 vs 70 wt% 最终核对 ★
2. PDMS 牌号(Sylgard 184?);固化温度与时间;真空脱泡参数
3. 充磁方式与场强、方向(脉冲充磁机?永磁体?)
4. PDMS_NdFeB 9 组合样品的纤毛间距
5. 形貌照片对应的显微镜型号与标定
6. Results-2 正文中 Ecoflex 措辞已全部改为 PDMS——如个别批次用过
   Ecoflex 请指出
