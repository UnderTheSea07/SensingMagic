...
NdFeB). \textbf{(b)}~Fabrication workflow --- placeholder pending vector
redraw. \textbf{(c)}~SEM micrographs: c1 array overview, c2 side view, c3
single-cilium cross-section (on-image 402.5~$\mu$m measurement; scale bars
to be added from SEM metadata). \textbf{(d)}~Stress--strain by magnetization
