...
\textbf{(c)} Geometry ($L=3,5,8$~mm; $d=0.5,0.8$~mm; pitch
...
$n=$~\dataneeded{5 (>=)}. \textbf{(e)} Geometry optimization and enhancement
vs.\ flat film, short pillars, non-magnetic cilia and bare sensor:
sensitivity heatmap over $L\times d$, cilia gain
$G_{\mathrm{cilia}}=\dataneeded{value + 95\% CI}$, LOD \dataneeded{mN},
sensitivity--range Pareto. \textbf{(f)} Dynamics/durability: step response,
frequency response to \dataneeded{Hz}, rise \dataneeded{ms} / recovery
\dataneeded{ms}, \dataneeded{$\geq$10{,}000} cycles, drift over
\dataneeded{$\geq$30 min}.
